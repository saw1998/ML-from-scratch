...
from sklearn.decomposition import PCA					#finding PCA using sklearn library
...
from Task_C import *
from Task_B import *							#importing K_mean and agglomerative algorithm
...
for i in sorted_cluster:					#storing results
...
for i in sorted_cluster:
    for j in range(len(i)):						#storing results
        f.write(str(i[j]))
        if(j!=len(i)-1):
            f.write(",")
    f.write("\n")
